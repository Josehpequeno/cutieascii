...
GOOS=linux GOARCH=amd64 go build -o $BUILD_DIR/$NAME cutieascii.go

# Criar tarball
echo "Creating distribution tarball..."
mkdir -p $BUILD_DIR/pkg/usr/bin
mkdir -p $BUILD_DIR/pkg/usr/share/$NAME
#Cria a estrutura de diretórios dentro de build/pkg que simula a estrutura de diretórios do Linux, com usr/bin para o binário e usr/share/cutieascii para os dados.
cp $BUILD_DIR/$NAME $BUILD_DIR/pkg/usr/bin/
cp -r kaoscii $BUILD_DIR/pkg/usr/share/$NAME/
...
Description: Display random cute ASCII art emojis
 A simple tool that displays random ASCII art emojis from kaoscii collection.
EOF

cp $BUILD_DIR/$NAME $BUILD_DIR/deb/usr/bin/
cp -r kaoscii $BUILD_DIR/deb/usr/share/$NAME/
